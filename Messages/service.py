...
from .models import MessageOutput, MessageInput
...
class MessageService:
    ...
    @classmethod
    async def info(cls, msg_id: int) -> MessageOutput:
        await DBCheckoutConnection()
        ...
        query = table.select()\
            .with_only_columns([table.c.id,
                                ...
                                table.c.text,
                                table.c.link,
                                array_agg(likes.c.usr_id)])\
            .select_from(table.join(likes,
                                    table.c.id == likes.c.msg_id,
                                    isouter=True))\
            .where(table.c.id == msg_id)\
            .group_by(table.c.id)

        data = await Database.fetch_one(query)
        like_list = data.get('array_agg_1')
        ...
        return MessageOutput(text=data.get('text'),
    ...
    @classmethod
    async def insert(cls, data: MessageInput, user: UserOutput)\
            -> MessageOutput:
        await DBCheckoutConnection()
        ...
        return MessageOutput(text=data.text,
                             media=data.media,
                             link=data.link,
                             usr_id=user.id,
                             id=msg_id,
    ...
    @classmethod
    async def delete(cls, msg_id: int, user: UserOutput) -> MessageOutput:
        await DBCheckoutConnection()
        ...
        return MessageOutput(text=msg.get('text'),
                             media=msg.get('media'),
                             link=msg.get('link'),
    ...
    async def like(cls, msg_id: int, user: UserOutput) -> bool:
        ...
        query = likes.select()\
            .where(likes.c.msg_id == msg_id)\
            .where(likes.c.usr_id == user.id)
